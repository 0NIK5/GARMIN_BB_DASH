...
### Шаг 2: Запустить все сервисы одним кликом (Windows)

Дважды кликнуть на **`start_all.bat`** в корне проекта — откроются три отдельных окна терминала (Backend, Frontend, Worker) и через 7 секунд автоматически откроется браузер на `http://127.0.0.1:5500`.

---

### Или запустить вручную — каждый сервис в отдельном терминале:
